...
cat > /config/do1.json <<EOF
{
    ...
    "Common": {
        ...
        "trust": {
            ...
            "localPassword": "2018F5Networks!!",
            "remoteHost": "-remote-peer-addr-",
            "remoteUsername": "xadmin",
            "remotePassword": "2018F5Networks!!"
        }
    }
}
EOF
...
cat > /config/as3.json <<EOF
{
    ...
    "declaration": { 
        ...
        "id": "aa293fdc-6acc-691c-c4c4-15791810959e",
        "label": "scca baseline",
                ...
                "sccaBaselineJuiceShop": {
                    ...
                    "members": [
                        ...
                        },
                        {
                            ...
                            "region": "us-east1",
                            "encodedCredentials": "-sd-sa-token-b64-"
                          }
                    ],
                    "class": "Pool"
                },
...
function runDO() {
    ...
    while [ $count -le 4 ]
        ...
        while true
        do
            doCodeType=\$(curl -s -u $CREDS -X GET $local_host/mgmt/shared/declarative-onboarding/task/\$task | jq -r type )
		    if [ "\$doCodeType" == "object" ]; then
            code=\$(curl -s -u $CREDS -X GET $local_host$doTaskUrl/\$task | jq -r .code)
                echo "object \$code"
            ...
            if [ -n "\$code" ]; then
                ...
                422)
                    # content error
                    echo "declaration content error retrying task: \$task"
                    break
                    ;;
                *)
                    # other
